...
int main ( int argc, char* argv[] ) {
    ...
    for (unsigned iter=0; iter<graphNodeCount-1; iter++) {
        ...
        for (graphNode_t source=0; source<graphNodeCount; source++) {
            if (parents[source]==-1) { // the parent needs to be connected
				continue;
            }

			for (graphNode_t dest=0; dest<graphNodeCount; dest++) {
				// printf("\nConsidering conn %ld -> %ld\n", source, dest);
				if (dest == source || parents[dest] != -1) {
					continue;
				}
				AdjacencyListNode* sourceDestConn = &(adjacencyList[source]);
				while (sourceDestConn != NULL && sourceDestConn->graphNode != dest) {
					sourceDestConn = sourceDestConn->next;
				}
				// This means this source is not connected to the dest
				if (sourceDestConn == NULL) {
					continue;
				}

				double weight = sourceDestConn->weight;

				// printf("Weight from %ld -> %ld: %f \n", dest, source, weight);
		...
		// printf("minDest: %ld, minSource: %ld\n", minDest, minSource);
